    ...
    <div class="bg-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('admin.matakuliah.update', $matakuliah->id) }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="nama" value="{{ $matakuliah->nama }}" class="w-full border rounded px-3 py-2"
                    required>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="sks" value="{{ $matakuliah->sks }}" class="w-full border rounded px-3 py-2"
                    min="1" required>
            </div>

            <div class="flex justify-between items-center">
                <a href="{{ route('admin.matakuliah.index') }}" class="text-sm text-gray-600 hover:underline">← Kembali</a>
                <button type="submit" class="bg-blue-500 text-white px-5 py-2 rounded hover:bg-blue-600">Simpan</button>
            </div>
        </form>
    </div>
@endsection