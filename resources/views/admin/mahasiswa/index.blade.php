        ...
        <div class="overflow-x-auto bg-white rounded-xl shadow ring-1 ring-gray-200">
            <table class="min-w-full text-sm text-left text-gray-700">
                <thead class="bg-blue-600 text-white text-xs uppercase sticky top-0 z-10">
                    <tr>
                        <th class="py-3 px-6 text-left">NRP</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($mahasiswa as $m)
                        <tr class="hover:bg-gray-50 transition-all duration-200">
                            <td class="py-4 px-6 whitespace-nowrap">{{ $m->nrp }}</td>
                            <td class="py-4 px-6 whitespace-nowrap">{{ $m->name }}</td>
                            <td class="py-4 px-6 whitespace-nowrap">{{ $m->email }}</td>
                            <td class="py-4 px-6 text-center space-x-3">
                                ...
                                <form action="{{ route('admin.mahasiswa.destroy', $m->id) }}" method="POST"
                                    class="inline" onsubmit="return confirm('Yakin hapus mahasiswa ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
